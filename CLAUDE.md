...
- Add a row to `drizzle/MIGRATION_CHECKLIST.md` for every new `E-` file and tick the per-environment boxes as you apply it. There is no auto-runner and the DBs drift — an unapplied migration surfaces as a runtime "column/relation does not exist" error (Drizzle lists every column in its INSERTs).
