...
## Cross-Module Build Order

Mirrors BRD §0.14 launch sequence. Each phase lists modules it advances.

| Phase | Duration   | Work                                                  | Advances    |
|-------|------------|-------------------------------------------------------|-------------|
| 0     | Pre-Wk 1   | Location master, dealer_leads.id type, AI dialer ack | Module 0/5  |
| 1     | Week 1     | All migrations + schema.ts mirror                     | Module 0    |
| 2     | Week 2     | Single-owner backend + Lead Detail + touchpoint form  | Module 1    |
| 3     | Week 2–3   | Status transitions + commercials + status history     | Module 1    |
| 4     | Week 3     | ASM mobile UI + visit logging + next_action chaining  | Module 2    |
| 5     | Week 4     | Escalations + admin dashboard + KPIs + CEO advisory   | Module 3/4  |
| 6     | Week 4–5   | Bulk upload + RBAC + onboarding integration           | Module 3    |
| 7     | Week 5     | UAT + bug fixes                                       | all         |
| 8     | Week 5–6   | Soft launch (1 IS rep, 1 ASM)                         | all         |
| 9     | Week 6     | Full rollout                                          | all         |

## Phase 0 Prerequisites & Open Items

| Blocker                                                | Owner            | Unblocks                            |
|--------------------------------------------------------|------------------|-------------------------------------|
| `master_locations` + `location_aliases` delivered      | Anirudh          | Bulk upload, territory routing, filters, reports |
| `dealer_leads.id` type confirmed (UUID vs Text)        | iTarang Tech     | All new Part 0 FK columns           |
| `dealer_onboarding_applications` schema additions      | Main BRD team    | Conversion → onboarding integration |
| AI dialer team confirms exclusion filter               | AI dialer team   | Section 0.2 contract / Module 5     |

### Deferred (per BRD §0.14)

- **V1.1:** NeoDove disposition sync, ASM SLA targets, pricing approval rule.
- **V1.2:** Interakt WhatsApp inbound webhook, outbound template registry, consent tracking.
- **V2:** Dormant auto-Lost cron, QC call sampling, fuzzy name dedup, auto-merge, language-based routing, mobile admin writes, auto-elevation to CEO, S3 recording archival, ASM bounce-back as system action.

## Files engineers will touch

| File / Folder                                                   | Why                                    |
|-----------------------------------------------------------------|----------------------------------------|
| `src/middleware.ts`                                             | New roles in role map + route guards   |
| `src/app/(auth)/login/page.tsx`                                 | Post-login redirect for new roles      |
| `src/components/layout/sidebar.tsx`                             | Per-role nav entries                   |
| `src/lib/auth-utils.ts`                                         | `requireRole` updates                  |
| `src/lib/db/schema.ts`                                          | Mirror every migration                 |
| `drizzle/E-XXX_*.sql`                                           | Hand-written, idempotent (CLAUDE.md)   |
| `src/lib/lifecycle/transitions.ts`                              | NEW — status machine                   |
| `src/lib/touchpoints/write.ts`                                  | NEW — single touchpoint writer         |
| `src/app/(dashboard)/inside-sales/`                             | NEW — Module 1 pages                   |
| `src/app/(dashboard)/asm/`                                      | NEW — Module 2 pages                   |
| `src/app/(dashboard)/admin/part0/`                              | NEW — Module 3 pages                   |
| `src/app/(dashboard)/ceo/escalations/`                          | NEW — Module 4 page                    |
| `src/app/api/inside-sales/`, `/asm/`, `/admin/part0/`, `/ceo/`  | NEW — API routes per module            |
