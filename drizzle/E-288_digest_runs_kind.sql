-- E-288: one digest ledger for many digest kinds.
--
-- WHY
--   E-287 shipped the twice-daily Dealer Validation digest. The KYC Review screen
...
--   ticker, this migration widens E-287's ledger by one column and the code
...
--
--   This is the same call E-283 made when it extended city_default_loan_products
...
--   because of exactly when it runs: E-287 was applied on 2026-09-07, the table
--   holds a single row, and NO DEPLOYED CODE READS IT -- the E-287 application
...
--   re-running is a no-op and a database that never had E-287 simply gets the
--   CREATE TABLE below.
--
-- THE CLAIM KEY WIDENS BY ONE COLUMN
--   E-287's partial unique index was on (digest_date, slot). Left in place it
...
--   Replacing a partial unique key has precedent: E-283 replaced E-282's
...
--   `kind` defaults to 'dealer_validation', which is what every pre-E-288 row is.
--   No backfill, no DML.
--
-- SAFE TO SKIP AT DEPLOY
--   Same as E-287: every read and write in src/lib/digests/engine.ts is
...
--
--   BUT NOT SKIPPABLE ALONGSIDE E-287's CODE: an environment carrying E-287's
--   table and E-288's CODE would fail every claim on `column "kind" does not
--   exist` (swallowed, so it degrades to silence). Apply this whenever the
--   digests code is deployed.
--
-- Idempotent -- safe to re-run, in either order relative to E-287.

-- 1. Rename E-287's table, if it is still under its old name.
DO $do$ BEGIN
...
-- 2. For a database that never had E-287 at all.
...
COMMENT ON TABLE digest_runs IS
  'E-288 (was dealer_validation_digest_runs, E-287): send ledger for every scheduled '
...
COMMENT ON COLUMN digest_runs.kind IS
  'E-288: which digest this row belongs to -- ''dealer_validation'' | ''kyc_review''. '
  'Must match a descriptor id in src/lib/digests/registry.ts. Defaults to '
  '''dealer_validation'' because that is what every row written before E-288 is.';

COMMENT ON COLUMN digest_runs.digest_date IS
  'E-287: the IST CALENDAR DAY the mail covers, not the day it was sent. The 09:00 '
...
COMMENT ON COLUMN digest_runs.slot IS
  'E-287: ''morning'' | ''evening'' | ''test''. Only the first two are covered by '
...
COMMENT ON COLUMN digest_runs.status IS
  'E-287: ''sending'' (claimed, in flight) | ''sent'' | ''failed''. A ''sent'' row is '
  'terminal and can never be reclaimed; that is what makes the slot once-a-day.';

COMMENT ON COLUMN digest_runs.attempts IS
  'E-287: how many times this slot has been claimed. The claim re-takes a ''failed'' '
...
COMMENT ON COLUMN digest_runs.counts IS
  'E-287: the figures that were actually mailed, kept so the settings screen can show '
...
COMMENT ON COLUMN digest_runs.triggered_by IS
  'E-287: ''ticker'' (src/instrumentation-node.ts, the primary path) | ''cron'' '
...
COMMENT ON COLUMN digest_runs.claimed_at IS
  'E-287: when the claim was taken. A row left in ''sending'' because the process died '
