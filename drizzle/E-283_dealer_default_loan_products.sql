------------------------------------------------------------------------------
-- E-283: default loan product per DEALER, and an admin-chosen priority.
--
-- PROBLEM. E-282 pins a default NBFC + loan product to a (state, city), so the
...
--
-- SHAPE. The E-282 table becomes a general rule table rather than a second one
...
--   NULL         Delhi       Delhi    any dealer, Delhi customers  (E-282)
--   NULL         Telangana   NULL     state-wide                   (E-282)
--
...
-- before an any-location one, then newest id. Every pre-existing E-282 row gets
-- priority 0, so with no dealer rules configured the resolution order is
-- identical to what it is today.
--
-- STILL NEVER WRONG. Unchanged from E-282: the pin is a filter over the BRE's
...
--
-- APPLY WITH E-282. Not independently skippable: the resolver selects
-- dealer_code and priority, so an environment carrying E-282 but not E-283
...
-- pre-E-282 full-matched-list behaviour.
--
-- Strictly additive and idempotent; re-running is a no-op.
------------------------------------------------------------------------------

-- accounts.id / leads.dealer_id. NULL = not dealer-scoped (E-282's behaviour).
...
-- The E-282 key allowed one ACTIVE row per (state, city), which would now
...
COMMENT ON COLUMN city_default_loan_products.dealer_code IS
  'E-283: accounts.id / leads.dealer_id (the dealer CODE varchar, not dealers.id). NULL = the rule is not dealer-scoped and applies to every dealer.';

COMMENT ON COLUMN city_default_loan_products.priority IS
  'E-283: highest wins. On a tie, specificity decides: dealer rule before location-only, exact city before state, located before any-location, then newest.';

COMMENT ON COLUMN city_default_loan_products.state IS
  'E-283: NULL = the rule declares no location and applies anywhere (only meaningful together with a dealer_code). NOT NULL since E-282 until this migration.';

COMMENT ON TABLE city_default_loan_products IS
  'E-282/E-283: pins one NBFC + loan product as the default offered to a dealer, a location, or both. Every scoping column is nullable and NULL means "any". Ordered by priority DESC then specificity, and applied by applyPinnedDefault() to the BRE hits — a pinned product that does not independently match is skipped and the next rule (or normal matching) is used.';
