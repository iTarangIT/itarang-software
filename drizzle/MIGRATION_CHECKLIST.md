...
| local / dev | `database-1.…` | the usual `.env.local` target. NO deployed app writes to it — a personal dev DB |
| sandbox | (Hostinger-served app → its own RDS) | genuinely separate; not db-1 and not db-2 |
...
>
> Conversely `db-1` is nobody's deployment: it sat 40.9h stale through a sandbox
> deploy, so a tick there says nothing about sandbox. Sandbox must be checked
> against its own RDS.
...
<!-- NUMBER COLLISION: E-185..E-188 exist twice — the risk-engine family
     ...
     Next free number is E-219 — E-218_expense_buckets took 218 on 2026-07-28.
     NOTE E-215 looks free on disk but is POISONED: it was applied to db-1 as
     E-215_expense_currency_conversion before that file was renamed to E-217, so
     reusing the number would collide with an already-applied name there.
