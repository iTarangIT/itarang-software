...
| E-189_battery_spec_models | battery_spec_models catalog (per-model rated voltage/capacity + under/over-voltage, over-current, over-temp display thresholds) + device_battery_map.battery_model logical FK — Intellicar electrical analytics judge packs against their model spec (resolution: spec > app_settings > env > defaults); seeds placeholder LFP-51V-105AH | ☐ | ✅ | ✅ | ☐ |
