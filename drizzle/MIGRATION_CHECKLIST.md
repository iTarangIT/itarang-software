...
| E-216_module_usage_user_daily | **Per-user module usage** behind the drill-down on `/operations/usage`: `module_usage_user_daily` at grain `(day, module, user_id)` with `role_at_ping`, `role_bucket`, `pings`, `sessions`, `last_ping_at`. **This REVERSES E-215's no-`user_id` decision** — by explicit product decision recorded in the migration header, not by oversight; read that header before extending it. It is the first table in the codebase that records WHERE a named person went inside the CRM, and it **covers external `dealer` / `nbfc_partner` accounts**, who E-215 counted in aggregate precisely because the aggregate could not name them — so the staff/partner notice is a prerequisite, not a formality. Mitigations follow E-214's conventions rather than the aggregate's: **30-day retention** (`MODULE_USER_RETENTION_DAYS`, pruned by `runDailySnapshot()`) and **read-audit** via `recordUsageView({module})`. Still no path, query, IP, user-agent, or sub-day timestamp. `module_usage_daily` is **kept and unchanged** — permanent, and the only thing that can answer a question about last quarter once this table has been pruned; both are written by ONE statement in `recordModuleUsage()` so they cannot disagree. `last_ping_at` also fixes the counter inflation E-215 shipped with: `recordModuleUsage()` had no equivalent of `recordHeartbeat()`'s 240-second guard, so module time ran ~12.5% ahead of "Time in CRM" on measured data (63 module pings vs 56 accepted session pings). The aggregate's `pings` increment is now conditional on this row accepting the ping, so one rule guards both. **`sessions` is deliberately NOT throttled** — a genuinely new session on the same module the same day is real. **No backfill is possible and none must be attempted**: the user→module link was never stored, and `module_visit_keys` must NOT be mined for it (it resolves to a session's OWNER, wrong for 6 of 6 rows tested on live data). Attribution starts the day this is applied. Storage params (`fillfactor=70` + autovacuum tuning); Drizzle cannot express them, so their absence from `schema.ts` is expected and not drift. Safe to ship before applying — `getModuleUsers()` never throws, so an unapplied E-216 renders the People section as "unavailable" and leaves the rest of the page working. Mirrored in `schema.ts`. | ☐ | ✅ | ☐ | ☐ |
...
<!-- NUMBER COLLISION: E-185..E-188 exist twice — the risk-engine family
     ...
     Next free number is E-217 (E-216 is module_usage_user_daily). Match on the FULL filename,
