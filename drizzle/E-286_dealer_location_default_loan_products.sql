------------------------------------------------------------------------------
-- E-286: the location on a default loan product rule is the DEALER's, not the
-- customer's.
--
-- NO DDL, NO DML. This file only rewrites the COMMENTs that E-282/E-283 put on
...
COMMENT ON TABLE city_default_loan_products IS
  'E-282/E-283/E-286: pins one NBFC + loan product as the default offered by a dealer, by DEALER LOCATION, or both. Every scoping column is nullable and NULL means "any". state/city are matched against accounts.state / accounts.city (the dealer''s own address) — NOT the customer''s. Ordered by priority DESC then specificity, and applied by applyPinnedDefault() to the BRE hits: a pinned product that does not independently match is skipped and the next rule (or normal matching) is used.';

COMMENT ON COLUMN city_default_loan_products.state IS
  'E-286: the DEALER''s state (accounts.state), not the customer''s. NULL = the rule declares no location and applies to a dealer anywhere (only meaningful together with a dealer_code).';

COMMENT ON COLUMN city_default_loan_products.city IS
  'E-286: the DEALER''s city (accounts.city), not the customer''s. NULL = every dealer city in `state`. An exact (state, city) row takes precedence over a state-wide one at equal priority.';

COMMENT ON COLUMN city_default_loan_products.dealer_code IS
  'E-283: accounts.id / leads.dealer_id (the dealer CODE varchar, not dealers.id). NULL = the rule is not dealer-scoped and applies to every dealer whose LOCATION matches. Since E-286 this column is also what the resolver joins accounts on to read that location.';
