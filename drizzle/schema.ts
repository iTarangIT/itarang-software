import { pgTable, index, foreignKey, unique, varchar, timestamp, integer, text, uuid, jsonb, date, numeric, boolean, bigint, serial, primaryKey, pgView, doublePrecision, pgSequence } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"


export const leadReferenceSeq = pgSequence("lead_reference_seq", {  startWith: "992", increment: "1", minValue: "1", maxValue: "9223372036854775807", cache: "1", cycle: false })
...
export const aiCallLogs = pgTable("ai_call_logs", {
	...
	index("ai_call_logs_call_id_idx").using("btree", table.callId.asc().nullsLast().op("text_ops")),
	index("idx_ai_call_logs_lead").using("btree", table.leadId.asc().nullsLast().op("timestamptz_ops"), table.createdAt.desc().nullsFirst().op("text_ops")),
	foreignKey({
			columns: [table.leadId],
			foreignColumns: [leads.id],
			name: "ai_call_logs_lead_id_fkey"
		}),
	unique("ai_call_logs_call_id_unique").on(table.callId),
]);
...
export const coBorrowerRequests = pgTable("co_borrower_requests", {
	...
	index("co_borrower_requests_lead_id_idx").using("btree", table.leadId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.createdBy],
			foreignColumns: [users.id],
			name: "co_borrower_requests_created_by_fkey"
		}),
	foreignKey({
			columns: [table.leadId],
			foreignColumns: [leads.id],
			name: "co_borrower_requests_lead_id_fkey"
		}).onDelete("cascade"),
]);
...
export const callSessions = pgTable("call_sessions", {
	...
	unique("call_sessions_session_id_key").on(table.sessionId),
]);
...
export const consentRecords = pgTable("consent_records", {
	...
	foreignKey({
			columns: [table.adminViewedBy],
			foreignColumns: [users.id],
			name: "consent_records_admin_viewed_by_fkey"
		}),
	foreignKey({
			columns: [table.leadId],
			foreignColumns: [leads.id],
...
export const inventory = pgTable("inventory", {
	...
	socLastSyncAt: timestamp("soc_last_sync_at", { withTimezone: true, mode: 'string' }),
}, (table) => [
	foreignKey({
			columns: [table.dealerId],
			foreignColumns: [accounts.id],
...
export const leads = pgTable("leads", {
	...
	index("idx_leads_ai_queue").using("btree", table.aiManaged.asc().nullsLast().op("int4_ops"), table.manualTakeover.asc().nullsLast().op("timestamptz_ops"), table.nextCallAt.asc().nullsLast().op("int4_ops"), table.callPriority.asc().nullsLast().op("timestamptz_ops")).where(sql`((ai_managed = true) AND (manual_takeover = false))`),
...
export const couponAuditLog = pgTable("coupon_audit_log", {
	...
	index("coupon_audit_log_action_idx").using("btree", table.action.asc().nullsLast().op("text_ops")),
	index("coupon_audit_log_coupon_idx").using("btree", table.couponId.asc().nullsLast().op("text_ops"), table.createdAt.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.couponId],
			foreignColumns: [couponCodes.id],
			name: "coupon_audit_log_coupon_id_fkey"
		}),
	foreignKey({
			columns: [table.leadId],
			foreignColumns: [leads.id],
			name: "coupon_audit_log_lead_id_fkey"
		}),
]);
...
export const digilockerTransactions = pgTable("digilocker_transactions", {
	...
	smsDeliveredAt: timestamp("sms_delivered_at", { withTimezone: true, mode: 'string' }),
	smsFailedReason: text("sms_failed_reason"),
	smsAttempts: integer("sms_attempts").default(0).notNull(),
	// TODO: failed to parse database type 'bytea'
	aadhaarPdf: unknown("aadhaar_pdf"),
}, (table) => [
...
export const scraperRunChunks = pgTable("scraper_run_chunks", {
	...
	index("scraper_run_chunks_run_id_idx").using("btree", table.runId.asc().nullsLast().op("text_ops")),
	index("scraper_run_chunks_status_idx").using("btree", table.status.asc().nullsLast().op("text_ops")),
]);
...
export const kycDataAudit = pgTable("kyc_data_audit", {
	...
	enteredAt: timestamp("entered_at", { withTimezone: true, mode: 'string' }).defaultNow(),
	reason: text(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("kyc_audit_lead_idx").using("btree", table.leadId.asc().nullsLast().op("text_ops")),
]);
...
export const otherDocumentRequests = pgTable("other_document_requests", {
	...
	foreignKey({
			...
			foreignColumns: [users.id],
			name: "other_document_requests_reviewed_by_fkey"
		}),
]);
...
export const nbfcTenants = pgTable("nbfc_tenants", {
	...
	unique("nbfc_tenants_slug_key").on(table.slug),
]);

export const nbfcLoans = pgTable("nbfc_loans", {
	...
	index("nbfc_loans_dpd_idx").using("btree", table.currentDpd.asc().nullsLast().op("int4_ops")),
	index("nbfc_loans_tenant_idx").using("btree", table.tenantId.asc().nullsLast().op("uuid_ops")),
	index("nbfc_loans_vno_idx").using("btree", table.vehicleno.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.loanApplicationId],
			foreignColumns: [loanApplications.id],
			name: "nbfc_loans_loan_application_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.tenantId],
			foreignColumns: [nbfcTenants.id],
			name: "nbfc_loans_tenant_id_fkey"
		}).onDelete("restrict"),
]);
...
export const riskCardRuns = pgTable("risk_card_runs", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	tenantId: uuid("tenant_id").notNull(),
	...
	index("risk_card_runs_severity_idx").using("btree", table.severity.asc().nullsLast().op("text_ops")),
	index("risk_card_runs_tenant_hyp_idx").using("btree", table.tenantId.asc().nullsLast().op("uuid_ops"), table.hypothesisId.asc().nullsLast().op("uuid_ops")),
	index("risk_card_runs_tenant_run_idx").using("btree", table.tenantId.asc().nullsLast().op("uuid_ops"), table.runAt.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
			columns: [table.tenantId],
			foreignColumns: [nbfcTenants.id],
			name: "risk_card_runs_tenant_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.hypothesisId],
			foreignColumns: [riskHypotheses.id],
			name: "risk_card_runs_hypothesis_id_fkey"
		}).onDelete("cascade"),
]);
...
export const riskHypotheses = pgTable("risk_hypotheses", {
	...
	unique("risk_hypotheses_slug_key").on(table.slug),
]);
...
export const afterSalesRecords = pgTable("after_sales_records", {
	...
	foreignKey({
			columns: [table.leadId],
			foreignColumns: [leads.id],
			name: "after_sales_records_lead_id_fkey"
		}).onDelete("set null"),
]);
...
export const productSelections = pgTable("product_selections", {
	...
	foreignKey({
			columns: [table.leadId],
			foreignColumns: [leads.id],
			name: "product_selections_lead_id_fkey"
		}).onDelete("cascade"),
]);
...
export const loanSanctions = pgTable("loan_sanctions", {
	...
	sanctionedAt: timestamp("sanctioned_at", { withTimezone: true, mode: 'string' }).defaultNow(),
	dealerApproved: boolean("dealer_approved").default(false),
	dealerApprovedAt: timestamp("dealer_approved_at", { withTimezone: true, mode: 'string' }),
	...
	foreignKey({
			columns: [table.leadId],
			foreignColumns: [leads.id],
			name: "loan_sanctions_lead_id_fkey"
		}).onDelete("cascade"),
]);
...
export const otpConfirmations = pgTable("otp_confirmations", {
	...
	foreignKey({
			columns: [table.leadId],
			foreignColumns: [leads.id],
			name: "otp_confirmations_lead_id_fkey"
		}).onDelete("cascade"),
]);
...
export const bolnaCalls = pgTable("bolna_calls", {
	...
	chunkReceivedAt: timestamp("chunk_received_at", { withTimezone: true, mode: 'string' }),
...
export const pdiRecords = pgTable("pdi_records", {
	...
	inspectedAt: timestamp("inspected_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
...
export const nbfcUsers = pgTable("nbfc_users", {
	userId: uuid("user_id").notNull(),
	tenantId: uuid("tenant_id").notNull(),
	role: varchar({ length: 32 }).default('viewer').notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("nbfc_users_tenant_idx").using("btree", table.tenantId.asc().nullsLast().op("uuid_ops")),
	index("nbfc_users_user_tenant_idx").using("btree", table.userId.asc().nullsLast().op("uuid_ops"), table.tenantId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
			name: "nbfc_users_user_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.tenantId],
			foreignColumns: [nbfcTenants.id],
			name: "nbfc_users_tenant_id_fkey"
		}).onDelete("cascade"),
	primaryKey({ columns: [table.userId, table.tenantId], name: "nbfc_users_pkey"}),
]);
