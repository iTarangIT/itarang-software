...
import {
  ...
  findApplicationIdByCompanyName,
...
async function fillDealerOnboardingWizard(
  page: Page,
  ...
  // STEP 1 — Company
  await page.locator('input[placeholder="Company Name"]').fill(COMPANY_NAME);
  ...
  await page.locator('input[placeholder="GST Number"]').fill(GSTIN);
  await page.locator('input[placeholder="Company PAN Number"]').fill(PAN);
  ...
  await page.locator('input[placeholder="Owner Name"]').fill(OWNER_NAME);
  await page.locator('input[placeholder="Owner Phone Number"]').fill(E2E_PHONE);
  await page.locator('input[placeholder="Owner Email ID"]').fill(OWNER_EMAIL);
  ...
  await page.locator('input[placeholder="Beneficiary Name"]').fill(OWNER_NAME);
  ...
  // If status=200 came back but the body fetch lost the race with the redirect,
  // the application IS in the DB. Look it up by the unique company name.
  if (submitBody.__unreadable) {
    if (submitBody.status >= 200 && submitBody.status < 300) {
      console.log('[Phase A.1] submit body unreadable but status 2xx — falling back to DB lookup by company name');
      await page.waitForTimeout(1_500);
      return await findApplicationIdByCompanyName(COMPANY_NAME);
    }
  ...
  if (!applicationId) {
    // Body said success but had no applicationId — fall back to DB lookup.
    return await findApplicationIdByCompanyName(COMPANY_NAME);
  }
