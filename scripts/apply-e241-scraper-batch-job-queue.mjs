...
//
// WHY THE LONG NAME. Two unrelated migrations both claimed E-241 — main's
// `E-241_offer_close_vocabulary.sql` and this branch's
...
// conflicting E-239 while that branch was open: main's was already merged AND
// applied, so it keeps the short name. `scripts/apply-e241.mjs` is the
// offer-close-vocabulary applier and is unchanged.
//
// The .sql filenames were deliberately NOT renumbered: this one was already
// applied to db-1/sandbox under the E-241 name on 2026-08-16, and renaming an
// applied migration buys nothing (there is no ledger in the database) while
// invalidating the apply record. See the note in MIGRATION_CHECKLIST.md.
//
