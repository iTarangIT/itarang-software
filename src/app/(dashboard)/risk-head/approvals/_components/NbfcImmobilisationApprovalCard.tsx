...
/**
 * NbfcImmobilisationApprovalCard — Risk Head approve/reject card.
 *
 * Same UI as the iTarang sales-head card, but posts to the NBFC-side
 * dual-approval routes (resolveActor session), so the NBFC Risk Head signs off
 * within their own portal. Approve dispatches the immobilisation execution;
 * reject requires a reason. The server enforces initiator≠approver and the
 * required_approver_role match.
 */
...
export default function NbfcImmobilisationApprovalCard(props: Props) {
  ...
  const expiresIn = props.expiresAt
    ? Math.max(
        0,
        Math.round((props.expiresAt.getTime() - Date.now()) / (60 * 60 * 1000)),
      )
    : null;
  ...
  async function approve() {
    ...
    try {
      const res = await fetch(
        `/api/nbfc/dual-approval/requests/${props.id}/approve`,
        {
          method: "POST",
  ...
  async function reject() {
    ...
    try {
      const res = await fetch(
        `/api/nbfc/dual-approval/requests/${props.id}/reject`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
      ...
      <dl className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-3 mb-4 text-sm">
        ...
        <Field
          label="Current DPD"
          value={props.currentDpd != null ? `${props.currentDpd}d` : "—"}
        />
      </dl>

      {error ? (
        <div className="mb-3 px-3 py-2 bg-red-50 text-red-700 text-xs rounded">
          {error}
        </div>
      ) : null}
