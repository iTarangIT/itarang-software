...
> = {
  hot:                { label: "Hot",          bg: "bg-red-50",     text: "text-red-600",     dot: "bg-red-500" },
  warm:               { label: "Warm",         bg: "bg-amber-50",   text: "text-amber-600",   dot: "bg-amber-400" },
  cold:               { label: "Cold",         bg: "bg-blue-50",    text: "text-blue-600",    dot: "bg-blue-400" },
  new:                { label: "New",          bg: "bg-gray-100",   text: "text-gray-600",    dot: "bg-gray-400" },
  qualified:          { label: "Qualified",    bg: "bg-emerald-50", text: "text-emerald-700", dot: "bg-emerald-500" },
  disqualified:       { label: "Disqualified", bg: "bg-zinc-100",   text: "text-zinc-500",    dot: "bg-zinc-400" },
  callback_requested: { label: "Callback",     bg: "bg-purple-50",  text: "text-purple-600",  dot: "bg-purple-500" },
  stop:               { label: "Stop",         bg: "bg-red-100",    text: "text-red-700",     dot: "bg-red-600" },
  completed:          { label: "Completed",    bg: "bg-teal-50",    text: "text-teal-700",    dot: "bg-teal-500" },
...
// Maps common column name variations to our schema fields
function normalizeRow(row: Record<string, string>) {
  ...
  return {
    shop_name:   get("shop_name", "shop name", "shopname", "business", "business name", "store", "store name"),
    dealer_name: get("dealer_name", "dealer name", "dealername", "name", "owner", "owner name", "contact"),
    phone:       get("phone", "mobile", "phone number", "mobile number", "contact number", "number"),
  ...
  const handleFile = async (f: File) => {
    ...
    try {
      ...
      if (f.name.endsWith(".csv")) {
        ...
        // Excel — use SheetJS via CDN dynamically
      ...
      if (normalized.length === 0) {
        setError("No valid rows found. Make sure your file has a 'phone' column.");
        return;
      }
          ...
          {status === "done" && result && (
            <div className="text-center py-6">
              ...
              <p className="text-sm text-gray-500">
                <span className="text-emerald-600 font-medium">{result.inserted} leads added</span>
                {result.skipped > 0 && (
                  <span className="text-gray-400"> · {result.skipped} skipped (duplicate phone)</span>
                )}
              </p>
              <button
            ...
            <>
              <div
                ...
                className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${
                  file ? "border-gray-300 bg-gray-50" : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"
                }`}
              >
                <input
                  ref={fileRef}
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-2 gap-1">
                    {[
                      ...
                      <div key={col} className="flex items-center gap-1.5 text-xs text-gray-500">
                  ...
                  <div className="border rounded-xl overflow-hidden">
                    <table className="w-full text-xs">
                      <thead className="bg-gray-50 border-b">
                        <tr>
                          {["shop_name", "dealer_name", "phone", "location"].map((h) => (
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {preview.map((row, i) => (
                          <tr key={i}>
                            <td className="px-3 py-2 text-gray-700 truncate max-w-[100px]">{row.shop_name || "—"}</td>
                            <td className="px-3 py-2 text-gray-700 truncate max-w-[100px]">{row.dealer_name || "—"}</td>
...
function AiDialerBanner({
  phase, currentLead, nextLead, countdown, callsMade, totalEligible, onStop, onSkipCountdown,
}: {
  phase: DialerPhase; currentLead: any | null; nextLead: any | null;
  countdown: number; callsMade: number; totalEligible: number;
  onStop: () => void; onSkipCountdown: () => void;
}) {
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-xl px-5 py-4 mb-4 flex items-center justify-between gap-4">
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 transition-colors ${phase === "calling" ? "bg-emerald-500" : "bg-amber-500"}`}>
          {phase === "calling"
          ...
          {phase === "calling" && currentLead && (
            <>
              <p className="text-sm font-semibold text-white truncate">
                Calling — {currentLead.shop_name || currentLead.dealer_name || "Lead"}
              </p>
              <p className="text-xs text-gray-400 mt-0.5 flex items-center gap-2 flex-wrap">
                <span className="flex items-center gap-1"><Phone className="w-3 h-3" />{currentLead.phone}</span>
                {currentLead.final_intent_score != null && (
                  <span className="flex items-center gap-1 text-emerald-400">
                    <TrendingUp className="w-3 h-3" /> Score {currentLead.final_intent_score}
                  </span>
                )}
              </p>
            </>
          ...
          {phase === "countdown" && (
            <>
              <p className="text-sm font-semibold text-white">
                Next call in <span className="text-amber-400 tabular-nums">{countdown}s</span>
              </p>
              {nextLead && (
                <p className="text-xs text-gray-400 mt-0.5 truncate">
                  Up next: {nextLead.shop_name || nextLead.dealer_name || "Lead"}
                  {nextLead.final_intent_score != null && ` · Score ${nextLead.final_intent_score}`}
                </p>
              )}
            </>
          )}
      ...
      <div className="hidden md:flex flex-col items-center gap-1.5 shrink-0">
        <p className="text-xs text-gray-400 tabular-nums">{callsMade} / {totalEligible} calls</p>
        <div className="w-36 h-1.5 bg-gray-700 rounded-full overflow-hidden">
          <div
            className="h-full bg-emerald-500 rounded-full transition-all duration-500"
            style={{ width: totalEligible > 0 ? `${(callsMade / totalEligible) * 100}%` : "0%" }}
          />
        </div>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        {phase === "countdown" && (
          <button onClick={onSkipCountdown} className="px-3 py-1.5 text-xs font-medium bg-amber-500 hover:bg-amber-600 text-white rounded-lg transition-all">
            Call Now
          </button>
        )}
        <button onClick={onStop} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-red-500 hover:bg-red-600 text-white rounded-lg transition-all">
...
export default function LeadsUnifiedPage() {
  ...
  const fetchLeads = useCallback(async (page: number, q: string) => {
    ...
    try {
      const res = await fetch(`/api/dealer-leads?page=${page}&limit=${LIMIT}&search=${encodeURIComponent(q)}`);
      const data = await res.json();
      if (data.success) { setLeads(data.leads); setLeadsTotal(data.total); }
  ...
  const fetchConvertedLeads = useCallback(async (page: number, q: string) => {
    ...
    try {
      const res = await fetch(`/api/scraper-leads/converted?page=${page}&limit=${LIMIT}&search=${encodeURIComponent(q)}`);
      const data = await res.json();
      if (data.success) { setConvertedLeads(data.leads); setConvertedTotal(data.total); }
  ...
  const handleSearch = (v: string) => { setSearch(v); setLeadsPage(1); setConvertedPage(1); };
  ...
  const triggerCall = useCallback(async (lead: any) => {
    ...
    } catch (e) { console.error("Dialer call error", e); }
  }, []);

  const startCountdownTo = useCallback((nextIdx: number) => {
    const queue = queueRef.current;
    if (stopRef.current || nextIdx >= queue.length) { setDialerPhase("idle"); setDialerOn(false); return; }
  ...
  const handleDialerOff = useCallback(() => {
    ...
    if (countdownRef.current) clearInterval(countdownRef.current);
    setDialerOn(false); setDialerPhase("idle");
    setDialerQueue([]); setDialerIndex(0); setDialerCallsMade(0);
    ...
    <div className="max-w-6xl mx-auto py-8 px-6 min-h-screen bg-gray-50">
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage dealer leads and track their interest</p>
        </div>
        <div className="flex items-center gap-3">
          {tab === "leads" && (
            ...
            >
              <div className={`relative w-9 h-5 rounded-full transition-colors duration-300 ${dialerOn ? "bg-emerald-500" : "bg-gray-200"}`}>
                <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-300 ${dialerOn ? "translate-x-4" : "translate-x-0"}`} />
              </div>
              <span className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
                <Zap className={`w-3.5 h-3.5 transition-colors ${dialerOn ? "text-emerald-500" : "text-gray-400"}`} />
                AI Dialer
              </span>
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full transition-colors ${dialerOn ? "bg-emerald-50 text-emerald-600" : "bg-gray-100 text-gray-400"}`}>
                {dialerOn ? "ON" : "OFF"}
              </span>
            </button>
          )}
      ...
      <div className="flex items-center justify-between mb-4 gap-4">
        <div className="flex items-center bg-white border border-gray-200 rounded-xl p-1 gap-1">
          {([ { key: "scraper", label: "Scraper" }, { key: "leads", label: "Leads" }, { key: "converted", label: "My Converted Leads" } ] as { key: Tab; label: string }[]).map(({ key, label }) => (
            <button
              key={key}
              onClick={() => { setTab(key); setSelectedRunId(null); }}
            ...
            <>
              <div className="space-y-2">
                {leads.map((lead) => {
                  ...
                  const lastOutcome = getLastOutcome(lead.follow_up_history ?? []);
                  const outcomeCfg = lastOutcome ? OUTCOME_CONFIG[lastOutcome] : null;
                  const nextCallLabel = formatNextCall(lead.next_call_at);
                  const isDisabled = NO_CALL_STATUSES.includes(lead.current_status ?? "");
                  const intentScore = lead.final_intent_score;
                  const isBeingCalled = dialerOn && dialerPhase === "calling" && currentDialerLead?.id === lead.id;
                  const isUpNext = dialerOn && dialerPhase === "countdown" && currentDialerLead?.id === lead.id;

                  return (
                    <div
                      key={lead.id}
                      className={`bg-white border rounded-xl px-5 py-4 transition-all duration-150 ${
                        isBeingCalled ? "border-emerald-400 ring-1 ring-emerald-300 shadow-sm"
                        : isUpNext ? "border-amber-300 ring-1 ring-amber-200"
                        : "border-gray-200 hover:border-gray-300 hover:shadow-sm"
                      }`}
                    >
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${isBeingCalled ? "bg-emerald-50" : isUpNext ? "bg-amber-50" : "bg-gray-100"}`}>
                            {isBeingCalled
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <p className="text-sm font-semibold text-gray-900 truncate">{lead.shop_name || "Unnamed Shop"}</p>
                              {isBeingCalled && (
                                <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-600 animate-pulse shrink-0">
                                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Calling…
                                </span>
                              )}
                              {isUpNext && (
                                <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-600 shrink-0">
                                  <Clock className="w-3 h-3" /> Up next in {countdown}s
                                </span>
                              )}
                            </div>
                            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                              <span className="flex items-center gap-1 text-xs text-gray-500"><User className="w-3 h-3" />{lead.dealer_name || "Unknown"}</span>
                              <span className="text-gray-300">·</span>
                              <span className="flex items-center gap-1 text-xs text-gray-400"><MapPin className="w-3 h-3" />{lead.location || "-"}</span>
                              <span className="text-gray-300">·</span>
                              <span className="flex items-center gap-1 text-xs text-gray-400"><Phone className="w-3 h-3" />{lead.phone || "-"}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                          ...
                          {intentScore != null && (
                            <span className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${getIntentBg(intentScore)} ${getIntentColor(intentScore)}`}>
                              <TrendingUp className="w-3 h-3" />{intentScore}
                            </span>
                          )}
                          {(lead.total_attempts ?? 0) > 0 && (
                            <span className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-500">
                              <RefreshCw className="w-3 h-3" />{lead.total_attempts}x
                            </span>
                          )}
                          {outcomeCfg && (
                            <span className={`text-xs font-medium ${outcomeCfg.color} hidden lg:block`}>{outcomeCfg.label}</span>
                          )}
                          {nextCallLabel && (
                            <span className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${nextCallLabel === "Overdue" ? "bg-red-50 text-red-600" : "bg-purple-50 text-purple-600"}`}>
                              <Clock className="w-3 h-3" />{nextCallLabel}
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <CallButton leadId={lead.id} phone={lead.phone ?? ""} disabled={isDisabled || !lead.phone} />
                          <Link href={`/leads/${lead.id}`}>
                            <button className="px-3 py-1.5 text-xs font-medium border border-gray-200 rounded-lg hover:bg-gray-50 transition-all">View</button>
                          </Link>
                          <Link href={`/leads/${lead.id}/edit`}>
                            <button className="px-3 py-1.5 text-xs font-medium text-gray-500 rounded-lg hover:bg-gray-50 transition-all">Edit</button>
                          </Link>
                        </div>
                      </div>
                    </div>
              ...
              {leads.length === 0 && <EmptyState label="No dealer leads found" />}
              <Pagination page={leadsPage} total={leadsTotal} limit={LIMIT} onChange={setLeadsPage} />
            </>
          )}
        </div>
      )}
      ...
      {tab === "converted" && (
        <div>
          {convertedLoading ? <LoadingSkeleton /> : (
            <>
              <div className="space-y-2">
                {convertedLeads.map((lead) => {
                  ...
                  const intentScore = lead.final_intent_score;
                  return (
                    <div key={lead.id} className="bg-white border border-gray-200 rounded-xl px-5 py-4 hover:border-gray-300 hover:shadow-sm transition-all duration-150">
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                              ...
                              <span className="flex items-center gap-1 text-xs text-gray-400"><Phone className="w-3 h-3" />{lead.phone || "-"}</span>
                              <span className="text-gray-300">·</span>
                              <span className="flex items-center gap-1 text-xs text-gray-400"><Calendar className="w-3 h-3" />{formatDate(lead.created_at)}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button
                            onClick={async () => {
                              ...
                              const data = await res.json();
                              alert(data.success ? "Lead assigned to Sales Head" : "Failed to assign lead");
                            }}
              ...
              {convertedLeads.length === 0 && <EmptyState label="No converted leads yet" />}
              <Pagination page={convertedPage} total={convertedTotal} limit={LIMIT} onChange={setConvertedPage} />
            </>
          )}
        </div>
      )}
