/**
 * POST /api/nbfc/acquire/[leadId]/kyc/coborrower/rc/verify
 *
 * NBFC-side mirror of the admin co-borrower RC-verify route. Runs the SAME
 * Decentro RC→chassis check for the lead's co-borrower against the shared
 * `kyc_verifications` row (applicant = co_borrower). No coupon side-effects.
 *
 * Body: { rc_number: string } — required (co_borrowers has no RC column).
 * Role: credit_underwriting | nbfc_admin, scoped to the acting tenant.
 * Response: the admin `{ success, data, error }` shape.
 */
