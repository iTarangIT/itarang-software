/**
 * POST /api/nbfc/acquire/[leadId]/kyc/cibil/score
 *
 * NBFC mirror of the admin primary CIBIL score route. Re-runs the applicant's
 * credit-bureau score via the shared @/lib/kyc/cibil-verification helper and
 * refreshes the shared kyc_verifications row (applicant='primary'). Unlike the
 * admin route it does NOT stamp the coupon / first-API-execution metadata.
 *
...
export async function POST(
  ...
  try {
    ...
    const result = await executeCibilScore(leadId, { applicant: "primary" });
