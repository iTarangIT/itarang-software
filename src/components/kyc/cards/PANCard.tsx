...
export default function PANCard({
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (!data.success)
        setError(data.message || data.error || "PAN verification failed");
    } catch {
      setStatus("failed");
  ...
  const handleAdminAction = async (
    ...
    try {
      ...
      if (data.success) {
        if (action === "accept") {
          setStatus("success");
          setActionResult({ success: true, message: "PAN verification accepted successfully" });
        } else if (action === "reject") {
          setStatus("failed");
          setActionResult({ success: true, message: "PAN verification rejected" });
        }
        onActionComplete?.();
      } else {
            ...
            <OcrAutofillButton
              ...
              onOcrResult={(data) => {
                ...
                for (const k of ["kycResult", "extractedData", "result", "ocrResult"]) {
                  if (data[k] && typeof data[k] === "object") searchTargets.push(data[k] as Record<string, unknown>);
                }
                for (const t of searchTargets) {
                  for (const f of ["pan_number", "panNumber", "id_number", "idNumber", "pan", "panNo"]) {
                    const v = t[f];
                    if (typeof v === "string" && v.trim()) { setPan(v.trim().toUpperCase()); return; }
                  }
                }
              }}
            />
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {crossMatchFields.map((f) => (
                    <tr
                      key={f.field}
                      className={`hover:bg-gray-50/50 ${!f.pass && f.matchScore !== null ? "bg-red-50/30" : ""}`}
                    >
          ...
          <div className="space-y-3 pt-4 border-t border-gray-200">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full mt-1.5 text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-teal-500 focus:border-teal-500 resize-none"
              />
            </div>
