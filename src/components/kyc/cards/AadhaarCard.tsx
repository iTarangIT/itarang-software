      ...
      <div className="p-5 space-y-5">
        ...
        {cardStatus === "awaiting_consent" && (
          <div className="space-y-4">
            ...
            {digilockerUrl && (() => {
              // Normalize phone to digits-only for deep links.
              // WhatsApp wa.me expects country-code + number, no "+".
              // sms: URL scheme keeps the "+" for E.164.
              const digitsOnly = (phone || "").replace(/\D/g, "");
              const waPhone = digitsOnly.length === 10 ? `91${digitsOnly}` : digitsOnly;
              const smsPhone = phone?.startsWith("+") ? phone : `+91${digitsOnly}`;
              const message = `Hi, please complete your Aadhaar verification for your loan application by opening this DigiLocker link: ${digilockerUrl}`;
              const waHref = `https://wa.me/${waPhone}?text=${encodeURIComponent(message)}`;
              const smsHref = `sms:${smsPhone}?body=${encodeURIComponent(message)}`;

              return (
                <div className="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
                  <div>
                    <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">DigiLocker Link (share with customer)</p>
                    <p className="text-[10px] text-amber-700 bg-amber-50 border border-amber-100 rounded px-2 py-1">
                      If the customer didn't receive the SMS from Decentro, share the link manually using the buttons below.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-gray-100">
                    <span className="text-[11px] text-gray-500 mr-1">Share via:</span>
                    <a href={waHref} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-green-600 hover:bg-green-700 rounded transition-colors"
                      title={`Send to ${phone} on WhatsApp`}>
                    ...
                    <a href={smsHref}
                      ...
                      title={`Open SMS to ${phone} on this device`}>
