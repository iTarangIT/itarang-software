...
export default function BankCard({
  ...
  const [validationType, setValidationType] = useState<ValidationType>("penniless");
    ...
    try {
      const res = await fetch(`/api/kyc/${leadId}/decentro/bank`, {
        ...
        body: JSON.stringify({
          ...
          validation_type: validationType === "pennydrop_name_match" ? "pennydrop" : validationType,
        }),
      });
      const data = await res.json();
      setResult(data);
      setStatus(data.success ? "success" : "failed");
      if (!data.success) setError(data.message || data.error || "Bank verification failed");
    } catch {
      setStatus("failed");
  ...
  const validationOptions: { value: ValidationType; label: string; cost: string }[] = [
    { value: "penniless", label: "Penniless", cost: "~Rs.1.50" },
    { value: "pennydrop", label: "Pennydrop", cost: "~Rs.1.50 + Rs.1" },
    { value: "pennydrop_name_match", label: "Pennydrop + Name Match", cost: "~Rs.1.50 + Rs.1" },
  ];
    ...
    <div className="border border-gray-200 rounded-xl bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-emerald-50 to-white">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-emerald-600 flex items-center justify-center text-white font-bold text-sm">B</div>
          <div>
            <h3 className="text-base font-semibold text-gray-900">Bank Account Verification</h3>
      ...
      <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={accountNumber} onChange={(e) => setAccountNumber(e.target.value)}
                placeholder="Enter account number" disabled={status === "loading"}
            ...
            <div>
              ...
              <input type="text" value={ifsc} onChange={(e) => setIfsc(e.target.value.toUpperCase())}
                placeholder="e.g. SBIN0001234" disabled={status === "loading"}
            ...
            <div>
              ...
              <input type="text" value={bankName} onChange={(e) => setBankName(e.target.value)}
                placeholder="Bank name" disabled={status === "loading"}
                className="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 text-sm disabled:bg-gray-100" />
            </div>
            <div>
              <label className="text-xs text-gray-500">Branch</label>
              <input type="text" value={branch} onChange={(e) => setBranch(e.target.value)}
                placeholder="Branch name" disabled={status === "loading"}
                className="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 text-sm disabled:bg-gray-100" />
            </div>
            <div>
              <label className="text-xs text-gray-500">Account Holder (Lead)</label>
              <input type="text" value={leadName} disabled
        ...
        <div>
          <p className="text-xs text-gray-500 uppercase font-semibold tracking-wide mb-2">Verification Method</p>
          <div className="grid grid-cols-3 gap-2">
            {validationOptions.map((opt) => (
              <button key={opt.value} onClick={() => setValidationType(opt.value)}
        ...
        {result && bankData && (
          <div>
            <p className="text-xs text-gray-500 uppercase font-semibold tracking-wide mb-3">Verification Results</p>
