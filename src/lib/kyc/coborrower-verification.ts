...
/**
 * Co-borrower variant of the DigiLocker resend-sms flow.
 *
 ...
 * (lead_id, verification_type='aadhaar', applicant='co_borrower'). This helper
 ...
 *
 * Returns `{ status, body }`; the calling route does
 *   NextResponse.json(result.body, { status: result.status }).
 */
export async function executeCoBorrowerDigilockerResendSms(
  leadId: string,
  ...
  const digilocker_url =
    typeof data.digilocker_url === "string" ? data.digilocker_url : null;
  const reference_id =
  ...
  if (!phone) {
    return {
      status: 400,
      body: { success: false, error: { message: "Co-borrower phone is missing." } },
    };
  }
  ...
  const smsAttempts =
    typeof data.sms_attempts === "number" ? data.sms_attempts : 0;
  ...
  const updatedData: Record<string, unknown> = {
    ...
    sms_delivered_at: smsResult.success
      ? now.toISOString()
      : (data.sms_delivered_at ?? null),
