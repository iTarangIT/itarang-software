...
async function propose(ctx: ToolContext, leadId: string, plan: SetFollowUpPlan, lines: Preview["lines"], warning: string | null) {
    ...
    const preview: Preview = {
        ...
        lines,
        // A note is not work (isWorkedTouchpoint): scheduling never resets the idle clock.
    ...
    const { id } = await createPending({
        ...
        preview,
        before: { next_follow_up_at: lead.next_follow_up_at?.toISOString() ?? null, asm_id: lead.asm_id },
