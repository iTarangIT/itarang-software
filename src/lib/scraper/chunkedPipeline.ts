...
export interface ChunkedRunOptions {
  // Explicit city list from a batch submission. When empty/omitted, cities are
  // generated by AI exactly as before.
  cities?: string[];
  // false ⇒ use baseQuery LITERALLY, one variation, no Gemini call at all.
  // Defaults to true so the two pre-E-241 call sites keep their behaviour.
  expandWithAi?: boolean;
  // Per-job result cap from the batch form / Excel `max_results` column.
  // Threaded to the chunk handler through the QStash message body — there is
  // deliberately no column for it on scraper_run_chunks (see below).
...
//
// `opts` is E-241. Both pre-existing call sites (/api/scraper/run and
// /api/scraper/cron) pass nothing and take the defaults, which reproduce the
// old behaviour exactly: AI-expanded queries crossed with AI-picked cities.
export async function startChunkedRun(
  runId: string,
  baseQuery: string,
  opts: ChunkedRunOptions = {},
) {
  const expandWithAi = opts.expandWithAi !== false;
  ...
  try {
    ...
    if (expandWithAi) {
      const cached = getCachedQueries(baseQuery);
      if (cached) {
        queryVariations = cached;
      } else {
      ...
      // The whole point of the batch form's default: no Gemini call, so cost is
      // a known constant (one chunk per job) and a 429 from the query generator
      // cannot fail a fan-out the operator has already paid for.
      queryVariations = [normalizeQuery(baseQuery)];
    }

    // An explicit city list is used verbatim — it is the operator's list, from
    // their textarea or their spreadsheet, and second-guessing it with AI is
    // exactly what the batch form exists to avoid.
    const cities = opts.cities?.length
      ? opts.cities
      : await generateCitiesForQuery(baseQuery);

    // E-227: keep the two components of the combination alongside it.
    // combination_query may itself contain " in " (e.g. "battery dealer in
    // mall in pune"), so it cannot be split back apart reliably afterwards.
    const combinations: {
      combo: string;
      queryText: string;
      city: string | null;
    ...
    const chunkRows = combinations.map((c, idx) => ({
      id: `CHUNK-${runId}-${String(idx).padStart(4, "0")}`,
      run_id: runId,
      combination_query: c.combo,
      status: "pending",
      leads_count: 0,
      query_text: c.queryText,
      city: c.city,
    }));
    ...
    const publishResults = await Promise.allSettled(
      chunkRows.map((chunk) =>
        publishToPath({
          ...
          // maxResults rides the message rather than a column: adding one to
          // scraper_run_chunks would mean adding it to the drizzle object too,
          // and drizzle names every column of that object in this very INSERT
          // — so an unapplied migration would break the fan-out itself. The
          // message body has no such coupling, and a chunk redelivered by
          // QStash still carries the value.
          body: { chunkId: chunk.id, maxResults },
        }),
      ),
    );
      ...
      const drain = async () => {
        while (queue.length) {
          ...
          await Promise.allSettled(
            batch.map((id) => executeChunk(id, { maxResults })),
          );
        }
      };
...
export async function executeChunk(
  ...
  // A per-job cap from the batch form. Google paginates ~20 results a page, so
  // the cap becomes a page budget; Apify takes a count directly. Clamped to the
  // pipeline's own ceiling either way — max_results can only ever ask for LESS
  // work than the default, never more.
  const cap =
    typeof opts.maxResults === "number" && opts.maxResults > 0
      ? opts.maxResults
      : null;
  const maxPages = cap
    ? Math.min(MAX_PAGES_PER_QUERY, Math.max(1, Math.ceil(cap / 20)))
    : MAX_PAGES_PER_QUERY;
  ...
  try {
    ...
    if (apifyEnabled) {
      tasks.push(
        fetchFromApifySingle(
          chunk.combination_query,
          cap ? { maxResults: cap } : {},
        ),
      );
    }
  ...
  // E-227 liveness heartbeat. Issued as its own best-effort statement rather
  // than folded into the counter bump below, because last_progress_at is not on
  // the drizzle object and a database without E-227 must lose the heartbeat,
  // not the chunk accounting. See touchRunProgress().
  await touchRunProgress(chunk.run_id);
