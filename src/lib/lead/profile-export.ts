/**
 * Customer profile / dossier export — shared ZIP + PDF builder.
 *
 * Extracted from the admin "Download Customer Profile" route (BRD V2 §2.6 / §3.3)
 * so the NBFC Acquire workspace can reuse the exact same bundle. Produces a ZIP
 * shaped to the BRD spec:
 ...
 *
 * The single behavioural knob is `maskPii`: the admin export masks
 * Aadhaar/PAN/account (true); the NBFC export shows them in full (false), since
 * the picked NBFC is the lender of record. No DB mutation.
 */
...
function maskPan(raw: string | null | undefined): string {
  if (!raw) return "—";
  return `${raw.slice(0, 5)}XXXX${raw.slice(-1)}`;
}

/** Conditional maskers — when `mask` is false, the raw value is shown verbatim. */
function aadhaarValue(raw: string | null | undefined, mask: boolean): string {
  if (!mask) return raw ?? "—";
  return maskAadhaar(raw);
}
function accountValue(raw: string | null | undefined, mask: boolean): string {
  if (!mask) return raw ?? "—";
  return maskAccount(raw);
}
function panValue(raw: string | null | undefined, mask: boolean): string {
  if (!mask) return raw ?? "—";
  return maskPan(raw);
}
...
interface CustomerProfileData {
  ...
  generatedAt: string;
  generatedBy: string;
  /** Mask Aadhaar/PAN/account in the rendered output. */
  mask: boolean;
}
...
function buildCustomerProfileHtml(d: CustomerProfileData): string {
  ...
  // — render whatever rows we have, masking sensitive values when `mask`.
    ...
    ${d.coBorrowerRows.length === 0 ? '<p class="muted">No co-borrower records found.</p>' :
      d.coBorrowerRows.map((cb) => `
        <table class="kv">
          ...
          <tr><th>PAN</th><td>${htmlEscape(panValue(cb.pan_no, mask))}</td></tr>
          <tr><th>Aadhaar</th><td>${htmlEscape(aadhaarValue(cb.aadhaar_no, mask))}</td></tr>
...
<body>
  ...
  <div class="footer">Generated by iTarang profile export · BRD V2 §2.6 / §3.3</div>
</body></html>`;
}
...
<body>
  ...
  <div class="footer">Generated by iTarang profile export · BRD V2 §2.6</div>
</body></html>`;
}

// ---------- orchestrator ----------------------------------------------------

/** Loads the lead row (used by callers that gate before building). */
export async function loadProfileLead(leadId: string) {
  const [lead] = await db.select().from(leads).where(eq(leads.id, leadId)).limit(1);
  return lead ?? null;
}

export interface BuildProfileZipOptions {
  /** Identifier recorded in the generated PDF ("Generated by"). */
  generatedBy: string;
  /** Mask Aadhaar/PAN/account in the PDFs (admin export = true, NBFC = false). */
  maskPii: boolean;
}

/**
 * Builds the full customer-profile ZIP for a lead. Returns null if the lead
 * does not exist. Performs no auth — callers own the auth/ownership gate.
 */
export async function buildProfileZip(
  leadId: string,
  opts: BuildProfileZipOptions,
): Promise<Buffer | null> {
  ...
  const dealerLookup = lead.dealer_id
    ...
    : [];
  const dealerRow = dealerLookup.find((u) => u.is_active) ?? dealerLookup[0] ?? null;
  ...
  // The consent doc (signed PDF) rendered in section 4 when manual consent was
  // used; pull the most recent one.
  const consentDoc =
    customerKycDocs
      .filter((doc) => (doc.doc_type || "").toLowerCase().includes("consent"))
      .sort((a, b) => {
  ...
  // Render both PDFs (sequential — the pooled browser page is churned twice).
  ...
  // /co_borrower_docs/ — co-borrower KYC files (sub-folder per co-borrower when
  // there are multiple, otherwise flat under /co_borrower_docs/).
  ...
  if (coBorrowerFolder && coBorrowerDocsRows.length > 0) {
    ...
    const cbNameById = new Map(coBorrowerRows.map((cb) => [cb.id, cb.full_name || cb.id]));
